eulerforzeroInc <- function(A1,A0,Y,Agrid1,dU1){
...
solveEulerEquationBCInc <- function(){
  ...
  for (ixA in 1:numPointsA){ # points on asset grid
    ...
    for (ixY in 1:numPointsY){ # points on income grid
      ...
      dU[T,ixA,ixY]<- getmargutility(policyC[T,ixA,ixY]) # marginal valye of policyC
  ...
  for (ixt in (T-1):1){ # Loop from time T-1 to 1
    ...
    for (ixA in 1:numPointsA){ # Loop through points on the asset grid
      ...
      for (ixY in 1:numPointsY){
        ...
        # compute solution
        signoflowerbound <- sign(eulerforzeroInc(A1=lbA1,A0=A,Y=Y,Agrid1=Agrid1,dU1=Edu1))
        if ((signoflowerbound==1) | (ubA1 - lbA1 < minCons) ){ # if liquidity constrained
          policyA1[ixt,ixA,ixY] <- lbA1
        } else{ # if interior solution
          signofupperbound <- sign(eulerforzeroInc(A1=ubA1,A0=A,Y=Y,Agrid1=Agrid1,dU1=Edu1))
          ...
          policyA1[ixt,ixA,ixY] <- fzero(eulerforzeroInc,A0=A,Y=Y,Agrid1=Agrid1,dU1=Edu1,bndForSol,tol=tol)$x
        }
        ...
        EV1 <- EV[ixt+1,,ixY]
        V[ixt,ixA,ixY] <- -objectivefunc(policyA1[ixt,ixA,ixY],A,Y,Agrid1,EV1)
      } # ixY

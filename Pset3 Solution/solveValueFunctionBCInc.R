...
objectivefunc <- function(A1,A0,Y,Agrid1,EV1){
...
solveValueFunctionBCInc <- function(){
    ...
    for (ixA in 1:numPointsA){ # points on asset grid
      ...
      for (ixY in 1:numPointsY){ # points on income grid
        ...
        if (ubA1 - lbA1 < minCons){ # if liquidity constrained
           negV <- objectivefunc(A1=lbA1,A0=A,Y=Y,Agrid1=Agrid1,EV1=EV1)
           policyA1[ixt,ixA,ixY] <- lbA1
        } else {
          oout <- fminbnd(objectivefunc,A0=A,Y=Y,Agrid1=Agrid1,EV1=EV1,lbA1,ubA1,tol=tol)

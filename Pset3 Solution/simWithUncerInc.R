  ...
  for (s in 1:numSims){
    ...
    for (t in 1:T){
      if (t < Tretire){
        ...
        tA1 <- policyA1[t,,1]
        tV  <- EV[t,,1]
        a[t+1,s] <- interp1(Agrid[t,],tA1,a[t,s],method=interpMethod)
        v[t  ,s] <- interp1(Agrid[t,],tV, a[t,s],method=interpMethod)
      } # if (t<Tretire)
